...
void ResidentList::searchByTransportLL(string target) {
    ...
    Node* current = head;
    while (current != nullptr) {
        if (current->data.transport == target) found++;
        current = current->next;
    }
    clock_t end = clock();
...
void ResidentList::searchByDistanceLL(float threshold) {
    clock_t start = clock();
    int found = 0;
    Node* current = head;
    while (current != nullptr) {
        if (current->data.distance > threshold) found++;
        current = current->next;
    }
    clock_t end = clock();
...
void ResidentList::searchByAgeRangeLL(int minAge, int maxAge) {
    clock_t start = clock();
    int found = 0;
    Node* current = head;
    while (current != nullptr) {
        if (current->data.age >= minAge && current->data.age <= maxAge) found++;
        current = current->next;
    }
    clock_t end = clock();
